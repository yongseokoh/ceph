...
void MDBalancer::handle_rank_mask(void)
{
  ...
  for (auto &cd : authsubs) {
    ...
    ceph_assert(target >= 0 && target < mds->mdsmap->get_max_mds());
...
void MDBalancer::handle_export_pins(void)
{
  ...
  while (it != q.end()) {
    ...
    if (export_pin == MDS_RANK_NONE) {
      std::string bal_rank_mask = in->get_rank_mask(false);
...
int MDBalancer::hex2bin(std::string bal_rank_mask, std::bitset<MAX_MDS>& temp_bitset)
{
  CachedStackStringStream css;
  std::string bin_string;
  int r;

  r = mds->get_mds_map()->hex2bin(bal_rank_mask, bin_string, MAX_MDS, *css);
  if (r == 0) {
    temp_bitset = std::bitset<MAX_MDS>(bin_string);
  } else {
