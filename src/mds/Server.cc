...
*/
  ...
  case CEPH_MSG_CLIENT_REQUEST:
      mds->mds_dmclock_scheduler->handle_mds_request(MClientRequest::msgref_cast(m));
    return;
...
void Server::handle_client_session(const MClientSession::const_ref &m)
{
  ...
  case CEPH_SESSION_REQUEST_OPEN:
	...
	dout(20) << "  " << p.first << ": " << p.second << dendl;
      ...
      dout(20) << " required client features" << required_client_features << dendl;
	...
	break;
      ...
      dout(20) << " missing features" << missing_features << dendl;
...
void Server::handle_conf_change(const std::set<std::string>& changed) {
    dout(0) << "Server::handle_conf_change() called" << dendl;
...
public:
  C_MDS_inode_update_finish(Server *s, MDRequestRef& r, CInode *i,
			    bool sm=false, bool cr=false, bool ar=false, bool ud=false) :
    ServerLogContext(s, r), in(i),
    truncating_smaller(sm), changed_ranges(cr), adjust_realm(ar), update_dmclock(ud) { }
  void finish(int r) override {
    ceph_assert(r == 0);
			       ...
			       MutationImpl::LockOpVec& lov)
{
  ...
  if (name.compare(0, 15, "ceph.dir.layout") == 0) {
    ...
    if (name.find("mds_") != std::string::npos) {
      ...
      string path_str;
      cur->make_path_string(path_str, true);
      filepath path(path_str);

      if (path.depth() != MDSDmclockScheduler::SUBVOL_ROOT_DEPTH) {
          dout(10) << "bad subvolume path " << path_str << " " << path.depth() << " for dmclock" << dendl;
          respond_to_request(mdr, -EINVAL);
          return;
      } else {
        filepath& parent_path = path;

        parent_path.pop_dentry();

        if (!(parent_path.get_ino() == MDS_INO_ROOT &&
            parent_path.get_path() == "volumes/_nogroup")) {
          dout(10) << "bad subvolume path " << path_str << " " << parent_path.depth() << " for dmclock" << dendl;
          respond_to_request(mdr, -EINVAL);
          return;
        }
      }
