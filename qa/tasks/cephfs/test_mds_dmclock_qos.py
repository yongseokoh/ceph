...
class TestMDSDmclockQoS(CephFSTestCase):
    ...
    def setUp(self):
        ...
        self.wait_until_equal(lambda: len(self.fs.get_active_names()), 2, 60,
                              reject_fn=lambda v: v > 2 or v < 1)
        self._fs_cmd("status")
        log.info("status = {0}".format(self.fs.status()))
        log.info("num mdss = {0}".format(len(self.fs.mds_ids)))
        ...
        for subv_ in self.subvolumes:
            ...
            subvolpath = self._fs_cmd("subvolume", "getpath", self.fs.name, subv_)
        ...
        self.mount_a.mount(mount_path=self.get_subvolume_path(self.subvolumes[0]))
        self.mount_b.mount(mount_path=self.get_subvolume_path(self.subvolumes[1]))

        # chown root to user
        from os import getuid, getgid
        self.mount_a.run_shell(['sudo', 'chown', "{0}:{1}".format(getuid(), getgid()), self.mount_a.mountpoint])
        self.mount_b.run_shell(['sudo', 'chown', "{0}:{1}".format(getuid(), getgid()), self.mount_b.mountpoint])
    ...
    def mds_asok_all(self, commands):
        ...
        for id in self.fs.get_active_names():
    ...
    def get_subvolume_root(self, mount):
        if mount == self.mount_a:
            return os.path.abspath(os.path.join(self.get_subvolume_path(self.subvolumes[0]), '../'))

        return os.path.abspath(os.path.join(self.get_subvolume_path(self.subvolumes[1]), '../'))

    def remount_to_mntpt(self, mount, mntpt):
        mount.umount_wait()
        mount.mount(mount_path=str(mntpt))

    def remount_subvolume_path_root(self, mount):
        mount.umount_wait()
        if mount == self.mount_a:
            mount.mount(mount_path=self.get_subvolume_path(self.subvolumes[0]))
        else:
            mount.mount(mount_path=self.get_subvolume_path(self.subvolumes[1]))

    def set_qos_xattr(self, mount, reservation, weight, limit):
        from os import getuid, getgid
        mount.run_shell(['sudo', 'chown', "{0}:{1}".format(getuid(), getgid()), mount.mountpoint])

        mount.setfattr(mount.mountpoint, "ceph.dmclock.mds_reservation", str(reservation))
        mount.setfattr(mount.mountpoint, "ceph.dmclock.mds_limit", str(limit))
        mount.setfattr(mount.mountpoint, "ceph.dmclock.mds_weight", str(weight))
    ...
    def dump_qos(self, id=None, ignore=["session_cnt", "session_list", "inflight_requests"]):
        ...
        for id_ in self.fs.get_active_names():
    ...
    def test_update_qos_value_root_volume(self):
        ...
        self.mount_a.umount_wait()
        self.mount_a.mount(mount_path="/")

        with self.assertRaises(CommandFailedError):
            self.mount_a.setfattr(self.mount_a.mountpoint, "ceph.dmclock.mds_reservation", str(reservation))
        with self.assertRaises(CommandFailedError):
            self.mount_a.setfattr(self.mount_a.mountpoint, "ceph.dmclock.mds_limit", str(limit))
        with self.assertRaises(CommandFailedError):
            self.mount_a.setfattr(self.mount_a.mountpoint, "ceph.dmclock.mds_weight", str(weight))

        # check whether values are none
        self.assertIsNone(self.mount_a.getfattr(self.mount_a.mountpoint, "ceph.dmclock.mds_reservation"))
        self.assertIsNone(self.mount_a.getfattr(self.mount_a.mountpoint, "ceph.dmclock.mds_limit"))
        self.assertIsNone(self.mount_a.getfattr(self.mount_a.mountpoint, "ceph.dmclock.mds_weight"))
    ...
    def test_update_qos_value_subvolume(self):
        ...
        self.assertEqual(
                int(float(self.mount_a.getfattr(self.mount_a.mountpoint, "ceph.dmclock.mds_reservation"))),
                reservation)
        self.assertEqual(
                int(float(self.mount_a.getfattr(self.mount_a.mountpoint, "ceph.dmclock.mds_weight"))),
                weight)
        self.assertEqual(
                int(float(self.mount_a.getfattr(self.mount_a.mountpoint, "ceph.dmclock.mds_limit"))),
                limit)
    ...
    def check_inode_cached(self, ino, mds_id):
        try:
            return self.fs.mds_asok(['dump', 'inode', str(ino)], mds_id=mds_id) 
        except ValueError:
            return None
    ...
    def test_verify_qos_value_on_cache_miss(self):
        ...
        if len(self.fs.get_active_names()) < 2:
            raise self.skipTest("To test multi MDSS, The number of MDS should be bigger than 2")
        ...
        log.info(self.mount_a.mountpoint)
        sub_ino = self.mount_a.path_to_ino("volumes/_nogroup/subvolume_0")
        log.info("/volumes/_nogroup/subvolume_0 inode {0}".format(sub_ino))

        nogroup_ino = self.mount_a.path_to_ino("volumes/_nogroup")
        log.info("/volumes/_nogroup inode {0}".format(nogroup_ino))
        ...
        while (self.check_inode_cached(sub_ino, 'a') is not None or
               self.check_inode_cached(sub_ino, 'b') is not None or
               self.check_inode_cached(nogroup_ino, 'a') is not None or
               self.check_inode_cached(nogroup_ino, 'b') is not None):

            self.mds_asok_all(["flush", "journal"])
            self.mds_asok_all(["flush_path", "/volumes/_nogroup"])
            time.sleep(10)

        self.assertIsNone(self.check_inode_cached(sub_ino, 'a'))
        self.assertIsNone(self.check_inode_cached(sub_ino, 'b'))

        self.assertIsNone(self.check_inode_cached(nogroup_ino, 'a'))
        self.assertIsNone(self.check_inode_cached(nogroup_ino, 'b'))

        self.mount_a.mount(mount_path=str(self.get_subvolume_root(self.mount_a)))
    ...
    def test_update_qos_value_dir(self):
        ...
        assert(self.mount_a.is_mounted())

        test_dir = path.join(self.mount_a.mountpoint, "test_dir")
        self.mount_a.run_shell(["mkdir", test_dir])

        reservation, weight, limit = 100, 100, 100

        # check subvolume path /volumes/_nogroup/subvolume
    ...
    def test_remote_update_qos_value(self):
        ...
        # check remote vxattr
        self.assertEqual(
                int(float(self.mount_b.getfattr(self.mount_b.mountpoint, "ceph.dmclock.mds_reservation"))),
                reservation)
        self.assertEqual(
                int(float(self.mount_b.getfattr(self.mount_b.mountpoint, "ceph.dmclock.mds_weight"))),
                weight)
        self.assertEqual(
                int(float(self.mount_b.getfattr(self.mount_b.mountpoint, "ceph.dmclock.mds_limit"))),
                limit)
    ...
    def test_pinning_each_rank_with_different_default_values(self):
        ...
        for rank in self.fs.get_ranks():
            if rank['name'] == 'a':
                self.mount_a.setfattr(self.mount_a.mountpoint, "ceph.dir.pin", str(rank['rank']))
            elif rank['name'] == 'b':
                self.mount_b.setfattr(self.mount_b.mountpoint, "ceph.dir.pin", str(rank['rank']))
    ...
    def test_mixed_mds(self):
        ...
        self.mount_a.setfattr(self.mount_a.mountpoint, "ceph.dir.pin", str(0))  # pinning to QoS enabled MDS
        self.mount_b.setfattr(self.mount_b.mountpoint, "ceph.dir.pin", str(1))  # pinning to QoS disabled MDS
    ...
    def test_qos_throttling_50IOPS(self):
        ...
        self.mount_a.setfattr(self.mount_a.mountpoint, "ceph.dir.pin", str(0))
...
def workload_create_files(tid, mount, test_cnt, results):
    ...
    base = path.join(mount.mountpoint, "thread_{0}".format(tid))
    mkdir(base)

    start = time.time()

    for i in range(test_cnt):
        touch(path.join(base, "file_{0}".format(i)))
